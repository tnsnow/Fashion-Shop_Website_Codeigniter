...
class MY_Model extends CI_Model {
	 ...
	 * $sql : cau lenh sql truyền vào câu lệnh sql 
	 */
	function query($sql){
	...
	protected function get_list_set_input($input = array())
	{
		...
		if ((isset($input['orwhere'][$key = ""])) && $input['orwhere'][$key = ""])
		{
			$this->db->or_where($input['orwhere'][$key = ""]);
		}
	...
	function showProduct($id_cate , $number)
	{
		$sql = " SELECT * FROM product 	WHERE catalog_id = {$id_cate} ORDER BY id LIMIT 0,$number";
	...
	public function showEnd($start = 0 ,$number)
	{
		$sql = " SELECT * FROM product 	WHERE 	total > 0 AND 	total < 5  LIMIT $start ,$number";
	...
	public function showSumEnd()
	{
		$sql = " SELECT * FROM product 	WHERE 	total > 0 AND 	total < 5 ";
	...
	public function getCreated(){
		$sql = " SELECT DISTINCT created  FROM transaction 	 ";
	...
	public function whereInPro($data, $limit) 
	{
		$sql = " SELECT * FROM product 	WHERE catalog_id In ($data)  LIMIT 0 ,$limit";
		$query = $this ->db-> query($sql);
		return $query->result();
	}
